...
def ResNet(split):
   ...
   # mean_file = root_str + 'imagenet_mean.binaryproto'
    ...
    if split == 'deploy':
        scale, result = conv_BN_scale_relu(split, bottom="data", nout = 16*k, ks = 3, stride = 1, pad = 1) #conv1
    else:
        scale, result = conv_BN_scale_relu(split, bottom=data, nout = 16*k, ks = 3, stride = 1, pad = 1) #conv1
    ...
    for i in range(repeat):             #conv2_x
        projection_stride = 1           #输入与输出的数据通道数都是 16，大小都是 32 X 32
                                        #可以直接相加，设置映射步长为 1
        result = ResNet_block(split, result, nout = 16*k, ks = 3, stride = 1, projection_stride =
    ...
    for i in range(repeat): #conv3_x
        ...
        result = ResNet_block(split, result, nout = 32*k, ks = 3, stride = 1, projection_stride
                              =projection_stride, pad = 1)
    ...
    for i in range(repeat): #conv3_x
        ...
        result = ResNet_block(split, result, nout = 64*k, ks = 3, stride = 1, projection_stride
                              =projection_stride, pad = 1)
...
def write_sovler():
    ...
    sovler_string.iter_size = 1                                        #每训练迭代test_interval次进行一次测试
