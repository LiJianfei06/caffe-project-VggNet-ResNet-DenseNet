...
root_str="/home/ljf/caffe-master/examples/ljftest_alphabet_VggNet/"
...
######################################
   ...
   #全连层
    if include_type=="deploy_fc":
        net.fc65_conv = caffe.layers.Convolution(net.pool8, kernel_size=4, num_output=1024, #stride=1,#pad=0,
                                         #param=[dict(lr_mult=1),dict(lr_mult=2)],
                                         #weight_filler=dict(type='xavier'),bias_filler=dict(type='constant')
                                         )
        net.relu65 = caffe.layers.ReLU(net.fc65_conv, in_place=True)        
    else:
        net.fc65 = caffe.layers.InnerProduct(net.pool8, num_output=1024,
...
def write_net():
    ...
    with open(train_proto, 'w') as f:
        f.write(str(create_net(train_lmdb,val_lmdb, mean_file, train_batch_size=512,val_batch_size=100)))
