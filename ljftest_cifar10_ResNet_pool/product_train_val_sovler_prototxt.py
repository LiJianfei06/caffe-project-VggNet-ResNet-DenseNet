...
def ResNet_block_pool(split, bottom, nout, ks, stride, projection_stride, pad):
    if projection_stride == 1: #1 代表不需要 1X1 的映射
        scale0 = bottom
    else: #否则经过 1X1，stride=2 的映射
        pool0 = L.Pooling(bottom, pool=caffe.params.Pooling.MAX, kernel_size=2, stride=2,pad=0) 
        scale0, relu0 = conv_BN_scale_relu(split, pool0, nout, 1, 1, 0)
...
#                                                 )))        
    ...
    for i in range(repeat):             #conv2_x
        projection_stride = 1           #输入与输出的数据通道数都是 16，大小都是 32 X 32
                                        #可以直接相加，设置映射步长为 1
        result = ResNet_block_pool(split, result, nout = 16, ks = 3, stride = 1, projection_stride =
    ...
    for i in range(repeat): #conv3_x
        ...
        result = ResNet_block_pool(split, result, nout = 32, ks = 3, stride = 1, projection_stride
                              =projection_stride, pad = 1)
    ...
    for i in range(repeat): #conv3_x
        ...
        result = ResNet_block_pool(split, result, nout = 64, ks = 3, stride = 1, projection_stride
                              =projection_stride, pad = 1)
