...
str_place="/home/ljf/caffe-master/examples/ljftest_cifar10_VggNet_BN/test/image/"
...
def test(my_project_root, deploy_proto):
    ...
    for dirpath, dirnames, filenames in os.walk(str_place):
        ...
        for filename in filenames:
            ...
            #if ((abs(ord(filename[0:3])-ord(labels[order].split(' ')[1]))==32 and ord(labels[order].split(' ')[1])>48) or (ord(filename[0])-ord(labels[order].split(' ')[1])==0)):
            if filename[0:3]==labels[order][0:3]:
            ...
            #print filename
            print("正确: %15s  预测: %10s "%(filename.split('.')[0], labels[order]))
