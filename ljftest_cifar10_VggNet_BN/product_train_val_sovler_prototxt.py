...
root_str="/home/ljf/caffe-master/examples/ljftest_cifar10_VggNet_BN/"
...
#######################################
    ...
    net.conv8 = caffe.layers.Convolution(net.pool7, kernel_size=3, stride=1,num_output=512, pad=1,
                                         param=[dict(lr_mult=1),dict(lr_mult=2)],
                                         weight_filler=dict(type='msra'), bias_filler=dict(type='constant'))
    net.BN8 = L.BatchNorm(net.conv8, batch_norm_param = dict(use_global_stats = use_global_stats),
                     in_place = True, #BN 的学习率惩罚设置为 0，由 scale 学习
                     param = [dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0)])
    net.scale8 = L.Scale(net.BN8, scale_param = dict(bias_term = True), in_place = True)
    net.relu8 = caffe.layers.ReLU(net.scale8, in_place=True)
...
#######################################
    net.conv9 = caffe.layers.Convolution(net.relu8, kernel_size=3, stride=1,num_output=512, pad=1,
                                         param=[dict(lr_mult=1),dict(lr_mult=2)],
                                         weight_filler=dict(type='msra'), bias_filler=dict(type='constant'))
    net.BN9 = L.BatchNorm(net.conv9, batch_norm_param = dict(use_global_stats = use_global_stats),
                     in_place = True, #BN 的学习率惩罚设置为 0，由 scale 学习
                     param = [dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0)])
    net.scale9 = L.Scale(net.BN9, scale_param = dict(bias_term = True), in_place = True)    
    net.relu9 = caffe.layers.ReLU(net.scale9, in_place=True)
#    #net.pool6 = caffe.layers.Pooling(net.relu6, pool=caffe.params.Pooling.MAX, kernel_size=2, stride=2,pad=0)  
##    net.norm6=caffe.layers.LRN(net.pool6,lrn_param = dict(local_size=5,alpha=0.0001,beta=0.75))    
########################################
    net.conv10 = caffe.layers.Convolution(net.relu9, kernel_size=3, stride=1,num_output=512, pad=1,
                                         param=[dict(lr_mult=1),dict(lr_mult=2)],
                                         weight_filler=dict(type='msra'), bias_filler=dict(type='constant'))
    net.BN10 = L.BatchNorm(net.conv10, batch_norm_param = dict(use_global_stats = use_global_stats),
                     in_place = True, #BN 的学习率惩罚设置为 0，由 scale 学习
                     param = [dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0)])
    net.scale10 = L.Scale(net.BN10, scale_param = dict(bias_term = True), in_place = True)       
    net.relu10 = caffe.layers.ReLU(net.scale10, in_place=True)
...
def write_net():
    ...
    with open(train_proto, 'w') as f:
        f.write(str(create_net(train_lmdb, mean_file, batch_size=256,include_type="train")))
