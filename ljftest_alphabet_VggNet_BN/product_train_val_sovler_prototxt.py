...
root_str="/home/ljf/caffe-master/examples/ljftest_alphabet_VggNet_BN/"

def create_net(file_lmdb, mean_file, file_batch_size=64,include_type="train"):
    ...
    if not include_type=="deploy":
        if not include_type=="deploy_fc":
            #第一层Data层
            if include_type=="train":
                net.data, net.label = caffe.layers.Data(name="face",source=file_lmdb, backend=caffe.params.Data.LMDB, batch_size=file_batch_size, ntop=2,   
...
#                                             weight_filler={"type": "xavier"},bias_filler={"type": "constant"})
    ...
    net.BN1 = L.BatchNorm(net.conv1, batch_norm_param = dict(use_global_stats = use_global_stats),
                     in_place = True, #BN 的学习率惩罚设置为 0，由 scale 学习
                     param = [dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0)])
    net.scale1 = L.Scale(net.BN1, scale_param = dict(bias_term = True), in_place = True)    
    #第三层ReLU激活层
    net.relu1 = caffe.layers.ReLU(net.scale1, in_place=True)
#    #第四层Pooling池化层
...
######################################
    ...
    net.BN2 = L.BatchNorm(net.conv2, batch_norm_param = dict(use_global_stats = use_global_stats),
                     in_place = True, #BN 的学习率惩罚设置为 0，由 scale 学习
                     param = [dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0)])
    net.scale2 = L.Scale(net.BN2, scale_param = dict(bias_term = True), in_place = True)      
    net.relu2 = caffe.layers.ReLU(net.scale2, in_place=True)
 ...
 ######################################
    ...
    net.BN3 = L.BatchNorm(net.conv3, batch_norm_param = dict(use_global_stats = use_global_stats),
                     in_place = True, #BN 的学习率惩罚设置为 0，由 scale 学习
                     param = [dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0)])
    net.scale3 = L.Scale(net.BN3, scale_param = dict(bias_term = True), in_place = True)  
    net.relu3 = caffe.layers.ReLU(net.scale3, in_place=True)
...
######################################
    ...
    net.conv4 = caffe.layers.Convolution(net.relu3, kernel_size=3, stride=1,num_output=128, pad=1,
                                         param=[dict(lr_mult=1),dict(lr_mult=2)],
                                         weight_filler=dict(type='gaussian',std=0.1),bias_filler=dict(type='constant',value=0.0))
    net.BN4 = L.BatchNorm(net.conv4, batch_norm_param = dict(use_global_stats = use_global_stats),
                     in_place = True, #BN 的学习率惩罚设置为 0，由 scale 学习
                     param = [dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0)])
    net.scale4 = L.Scale(net.BN4, scale_param = dict(bias_term = True), in_place = True)  
    net.relu4 = caffe.layers.ReLU(net.scale4, in_place=True)
...
######################################
    net.conv5 = caffe.layers.Convolution(net.pool4, kernel_size=3, stride=1,num_output=256, pad=1,
                                         param=[dict(lr_mult=1),dict(lr_mult=2)],
                                         weight_filler=dict(type='gaussian',std=0.1),bias_filler=dict(type='constant',value=0))
    net.BN5 = L.BatchNorm(net.conv5, batch_norm_param = dict(use_global_stats = use_global_stats),
                     in_place = True, #BN 的学习率惩罚设置为 0，由 scale 学习
                     param = [dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0)])
    net.scale5 = L.Scale(net.BN5, scale_param = dict(bias_term = True), in_place = True)  
    net.relu5 = caffe.layers.ReLU(net.scale5, in_place=True)
...
#######################################
    net.conv6 = caffe.layers.Convolution(net.relu5, kernel_size=3, stride=1,num_output=256, pad=1,
                                         param=[dict(lr_mult=1),dict(lr_mult=2)],
                                         weight_filler=dict(type='gaussian',std=0.1),bias_filler=dict(type='constant',value=0))
    net.BN6 = L.BatchNorm(net.conv6, batch_norm_param = dict(use_global_stats = use_global_stats),
                     in_place = True, #BN 的学习率惩罚设置为 0，由 scale 学习
                     param = [dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0)])
    net.scale6 = L.Scale(net.BN6, scale_param = dict(bias_term = True), in_place = True) 
    net.relu6 = caffe.layers.ReLU(net.scale6, in_place=True)
...
#######################################
    net.conv7 = caffe.layers.Convolution(net.relu6, kernel_size=3, stride=1,num_output=256, pad=1,
                                         param=[dict(lr_mult=1),dict(lr_mult=2)],
                                         weight_filler=dict(type='gaussian',std=0.1),bias_filler=dict(type='constant',value=0))
    net.BN7 = L.BatchNorm(net.conv7, batch_norm_param = dict(use_global_stats = use_global_stats),
                     in_place = True, #BN 的学习率惩罚设置为 0，由 scale 学习
                     param = [dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0),
                              dict(lr_mult = 0, decay_mult = 0)])
    net.scale7 = L.Scale(net.BN7, scale_param = dict(bias_term = True), in_place = True) 
    net.relu7 = caffe.layers.ReLU(net.scale7, in_place=True)
    net.pool7 = caffe.layers.Pooling(net.relu7, pool=caffe.params.Pooling.MAX, kernel_size=2, stride=2,pad=0)  
#######################################
#
#    net.conv8 = caffe.layers.Convolution(net.relu7, kernel_size=1, stride=1,num_output=256, pad=0,
#                                         param=[dict(lr_mult=1),dict(lr_mult=2)],
#                                         weight_filler=dict(type='gaussian',std=0.1),bias_filler=dict(type='constant',value=0))
...
##    net.norm6=caffe.layers.LRN(net.pool6,lrn_param = dict(local_size=5,alpha=0.0001,beta=0.75))    
########################################
#    net.conv10 = caffe.layers.Convolution(net.relu9, kernel_size=1, stride=1,num_output=512, pad=0,
#                                         param=[dict(lr_mult=1),dict(lr_mult=2)],
#                                         weight_filler=dict(type='gaussian',std=0.1),bias_filler=dict(type='constant',value=0))
...
######################################
   ...
   #全连层
    if include_type=="deploy_fc":
        net.fc65_conv = caffe.layers.Convolution(net.pool7, kernel_size=4, num_output=256, #stride=1,#pad=0,
                                         #param=[dict(lr_mult=1),dict(lr_mult=2)],
                                         #weight_filler=dict(type='xavier'),bias_filler=dict(type='constant')
                                         )
        net.relu65 = caffe.layers.ReLU(net.fc65_conv, in_place=True)        
    else:
        net.fc65 = caffe.layers.InnerProduct(net.pool7, num_output=256,
                                            weight_filler=dict(type='gaussian',std=1.0/256.0),bias_filler=dict(type='constant',value=0.1))
...
######################################
   #全连层
    if include_type=="deploy_fc":
        net.fc66_conv = caffe.layers.Convolution(net.relu65, kernel_size=1, num_output=256, #stride=1,#pad=0,
                                         #param=[dict(lr_mult=1),dict(lr_mult=2)],
                                         #weight_filler=dict(type='xavier'),bias_filler=dict(type='constant')
                                         )
        net.relu66 = caffe.layers.ReLU(net.fc66_conv, in_place=True)        
    else:
        net.fc66 = caffe.layers.InnerProduct(net.drop65, num_output=256,
                                            weight_filler=dict(type='gaussian',std=1.0/256.0),bias_filler=dict(type='constant',value=0.1))
...
######################################
   #创建一个dropout层
    if include_type=="deploy_fc":
        ...
        net.fc100_conv = caffe.layers.Convolution(net.relu66 , kernel_size=1, num_output=62,#stride=1, #pad=0,
                                         #param=[dict(lr_mult=1),dict(lr_mult=2)],
                                         #weight_filler=dict(type='xavier'),bias_filler=dict(type='constant')
                                         )
    else:
        net.fc100 = caffe.layers.InnerProduct(net.drop66, num_output=62,
                                            weight_filler=dict(type='gaussian',std=0.1),bias_filler=dict(type='constant',value=0))
    #创建一个softmax层
    if not include_type=="deploy":
        if not include_type=="deploy_fc":
    ...
    if include_type=="deploy_fc":
        ...
        return str(net.to_proto())    
    if include_type == 'test':
        net.acc = caffe.layers.Accuracy(net.fc100, net.label)
    return str(net.to_proto())
def write_net():
    ...
    val_proto = caffe_root + "test.prototxt"                        #保存test_prototxt文件的位置
    #写入prototxt文件
    with open(train_proto, 'w') as f:
        f.write(str(create_net(file_lmdb=train_lmdb, mean_file=mean_file, file_batch_size=800, include_type="train")))

    #写入prototxt文件
    with open(val_proto, 'w') as f:
        f.write(str(create_net(file_lmdb=val_lmdb,mean_file=mean_file, file_batch_size=100, include_type="test")))
...
def write_sovler():
    ...
    sovler_string.snapshot = 1000                                             #保存临时模型的迭代数
